...
uint64_t dtm_t::dtm_write_csr(unsigned which, uint64_t data)
{
  return dtm_modify_csr(which, data, WRITE);
}

uint64_t dtm_t::dtm_set_csr(unsigned which, uint64_t data)
{
  return dtm_modify_csr(which, data, SET);
}

uint64_t dtm_t::dtm_clear_csr(unsigned which, uint64_t data)
{
  return dtm_modify_csr(which, data, CLEAR);
}

uint64_t dtm_t::dtm_read_csr(unsigned which)
{
  return dtm_set_csr(which, 0);
}

uint64_t dtm_t::dtm_modify_csr(unsigned which, uint64_t data, uint32_t type)
{
  halt(current_hart);
...
void dtm_t::reset()
{
  for (int hartsel = 0; hartsel < num_harts; hartsel ++ ){
    ...
    dtm_write_csr(0x7b1, get_entry_point());
  }
...
void dtm_t::producer_thread()
{
  ...
  // These things are only needed for the 'dtm_modify_csr' function.
