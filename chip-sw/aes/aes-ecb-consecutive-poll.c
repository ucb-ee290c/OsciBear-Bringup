...
int main(void)
{
  ...
  // ----------------- Consecutive Block 128b Enc/Dec Loop -----------------
  printf("AES E/D Loop, Consecutive Block, 128b Key, Polling:\n");

  // Encrypting all four blocks using consecutive block operations
  printf("Encrypting plaintext and checking output...\n");
  AES_ECB_ENC_POLL(keyone, 128, plaintext, fillerspace, 16);
  for (int i = 1; i < 4; i++) {
      AES_ECB_NO_KEY_ENC_POLL(plaintext + (i * 16), fillerspace + (i * 16), 16);
  }

  // Comparing actual results to expected
  checkBytes(fillerspace, ciphertextone, 64);

  // Decrypting all four blocks using consecutive block operations (using expected as source)
  printf("Decrypting ciphertext and checking output...\n");
  AES_ECB_DEC_POLL(keyone, 128, ciphertextone, fillerspace, 16);
  for (int i = 1; i < 4; i++) {
      AES_ECB_NO_KEY_DEC_POLL(ciphertextone + (i * 16), fillerspace + (i * 16), 16);
  }

  // Comparing actual results to expected
  checkBytes(fillerspace, plaintext, 64);

  // ----------------- Consecutive Block 256b Enc/Dec Loop -----------------
  printf("\nAES E/D Loop, Consecutive Block, 256b Key, Polling:\n");

  // Encrypting all four blocks using consecutive block operations
  printf("Encrypting plaintext and checking output...\n");
  AES_ECB_ENC_POLL(keytwo, 256, plaintext, fillerspace, 16);
  for (int i = 1; i < 4; i++) {
      AES_ECB_NO_KEY_ENC_POLL(plaintext + (i * 16), fillerspace + (i * 16), 16);
  }
  ...
  // Decrypting all four blocks using consecutive block operations (using expected as source)
  printf("Decrypting ciphertext and checking output...\n");
  AES_ECB_DEC_POLL(keytwo, 256, ciphertexttwo, fillerspace, 16);
  for (int i = 1; i < 4; i++) {
      AES_ECB_NO_KEY_DEC_POLL(ciphertexttwo + (i * 16), fillerspace + (i * 16), 16);
  }
