...
int main(void)
{
  ...
  // ----------------- Multiple Block 128b Enc/Dec Loop -----------------
  printf("AES E/D Loop, Multiple Block, 128b Key, Polling:\n");

  // Encrypting all four blocks using multiple block operation
  printf("Encrypting plaintext and checking output...\n");
  AES_ECB_ENC_POLL(keyone, 128, plaintext, fillerspace, 64);

  // Comparing actual results to expected
  checkBytes(fillerspace, ciphertextone, 64);

  // Decrypting all four blocks using multiple block operation (using expected as source)
  printf("Decrypting ciphertext and checking output...\n");
  AES_ECB_DEC_POLL(keyone, 128, ciphertextone, fillerspace, 64);

  // Comparing actual results to expected
  checkBytes(fillerspace, plaintext, 64);

  // ----------------- Multiple Block 256b Enc/Dec Loop -----------------
  printf("\nAES E/D Loop, Multiple Block, 256b Key, Polling:\n");

  // Encrypting all four blocks using multiple block operation
  printf("Encrypting plaintext and checking output...\n");
  AES_ECB_ENC_POLL(keytwo, 256, plaintext, fillerspace, 64);
  ...
  // Decrypting all four blocks using multiple block operation (using expected as source)
  printf("Decrypting ciphertext and checking output...\n");
  AES_ECB_DEC_POLL(keytwo, 256, ciphertexttwo, fillerspace, 64);
